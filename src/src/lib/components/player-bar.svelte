  ...
  >
    ...
    <div class="flex items-center gap-3 w-56 shrink-0">
      <div class="size-12 shrink-0 rounded-md bg-muted overflow-hidden">
        {#if playerService.currentSong.avatarUrl}
          <img src={playerService.currentSong.avatarUrl} alt="" class="size-full object-cover" />
        {:else}
          <div class="size-full bg-gradient-to-br from-primary/30 to-primary/10" />
        {/if}
      </div>
      <div class="min-w-0">
        <p class="text-sm font-medium truncate">{playerService.currentSong.name}</p>
        <p class="text-xs text-muted-foreground truncate">{playerService.currentSong.artistsName}</p>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col items-center gap-1 max-w-xl mx-auto">
      ...
      <div class="flex items-center gap-1">
        <Button variant="ghost" size="icon" class="cursor-pointer" onclick={togglePlayMode} title={playModeLabel}>
          <span class="text-[10px] font-bold text-muted-foreground">{playModeLabel}</span>
        </Button>
        <Button variant="ghost" size="icon" class="cursor-pointer" onclick={() => playerService.prev()}>
        ...
        <Button variant="ghost" size="icon" class="cursor-pointer" onclick={() => playerService.next()}>
          <SkipForward class="size-5" />
        </Button>
        <Button variant="ghost" size="icon" class="cursor-pointer" onclick={() => showLyrics = !showLyrics}>
