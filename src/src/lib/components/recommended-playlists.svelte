<script lang="ts">
  ...
  $effect(() => {
    ...
    loaded = true;
    Promise.all(online.map((a) => getRecommendedPlaylists(a.slug, 4).catch(() => [] as Playlist[])))
      .then((results) => { playlists = results.flat().slice(0, 4); })
...
<div class="flex flex-col gap-3 min-h-0 overflow-hidden">
  ...
  <ScrollArea class="flex flex-col gap-2 flex-1 min-h-0">
    ...
    {:else}
      {#each playlists as p}
        <div class="mb-2">
          <PlaylistRowCard playlist={p} onplay={() => goto(`/adapter/${p.adapterSlug}/playlist/${encodeURIComponent(p.id)}`)} />
        </div>
      {/each}
    {/if}
  </ScrollArea>
