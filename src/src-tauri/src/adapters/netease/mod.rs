...
impl NeteaseAdapter {
    ...
    /// Bootstrap an anonymous token. Must be called once before any API
    /// calls that require authentication (search, song URLs, etc.).
    pub async fn bootstrap(&self) -> Result<()> {
        self.client.bootstrap_anonymous().await
    }
}
...
impl Adapter for NeteaseAdapter {
    ...
    async fn search(&self, keyword: &str) -> Result<SearchResult> {
        ...
        //   crypto: 'eapi'
        //   data: { s, type, limit, offset, total }
